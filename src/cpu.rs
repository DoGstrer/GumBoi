...
impl Registers{
    ...
    fn get_hl(&self) -> u16{ ((self.h as u16 )<< 8|self.l as u16).into() }
    fn set_hl(&mut self,value:u16){ self.h=((value&(0x00ff<<8))>>8) as u8; self.l=(value&0x00ff) as u8; }
    fn get_bc(&self) -> u16{ ((self.b as u16 )<< 8|self.c as u16).into() }
    fn set_bc(&mut self,value:u16){ self.b=((value&(0x00ff<<8))>>8) as u8; self.c=(value&0x00ff) as u8; }
    fn get_de(&self) -> u16{ ((self.d as u16 )<< 8|self.e as u16).into() }
    fn set_de(&mut self,value:u16){ self.d=((value&(0x00ff<<8))>>8) as u8; self.e=(value&0x00ff) as u8; }
    fn get_af(&self) -> u16{ ((self.a as u16 )<< 8|self.f as u16).into() }
    fn set_af(&mut self,value:u16){ self.a=((value&(0x00ff<<8))>>8) as u8; self.f=(value&0x00ff) as u8; }
    
    //flag operations
    fn set_z(&mut self){ self.f|=SET_Z; }
    fn reset_z(&mut self){ self.f&=!SET_Z; }
    fn get_z(&self) -> bool { self.f&SET_Z==SET_Z }
    fn set_n(&mut self){ self.f|=SET_N; }
    fn reset_n(&mut self){ self.f&=!SET_N; }
    fn get_n(&self) -> bool { self.f&SET_N==SET_N }
    fn set_h(&mut self){ self.f|=SET_H; }
    fn reset_h(&mut self){ self.f&=!SET_H; }
    fn get_h(&self) -> bool { self.f&SET_H==SET_H }
    fn set_c(&mut self){ self.f|=SET_C; }
    fn reset_c(&mut self){ self.f&=!SET_C; }
    fn get_c(&self) -> bool { self.f&SET_C==SET_C }
...
impl<'life> CPU{
    pub fn initialize() ->CPU{
        CPU{
            ...
            is_halt:false,
            is_stop:false,
            gb_clock:Duration::from_nanos(1000000),
        }
    }
    pub fn execute(&mut self){
        let opcode:u8 = self.memory_bus.get_addr(self.registers.pc);
        let mut opcode_cb:u8=0x0;
        let mut byte:u16=0x0;
        let mut byte8:u8=0x0;
        let mut flag:bool=false;
        let execution_time_now = time::Instant::now();

        match opcode{
            //8 bit LD
            0x06 => { self.registers.pc+=1; self.registers.b=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            0x0E => { self.registers.pc+=1; self.registers.c=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            0x16 => { self.registers.pc+=1; self.registers.d=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            0x1E => { self.registers.pc+=1; self.registers.e=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            0x26 => { self.registers.pc+=1; self.registers.h=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            0x2E => { self.registers.pc+=1; self.registers.l=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            ...
            0x7E => { self.registers.a=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            ...
            0x46 => { self.registers.b=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            ...
            0x4E => { self.registers.c=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            ...
            0x56 => { self.registers.d=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            ...
            0x5E => { self.registers.e=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            ...
            0x66 => { self.registers.h=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            ...
            0x6E => { self.registers.l=self.memory_bus.get_addr(self.registers.get_hl()); self.cycle=8; },
            0x70 => { self.memory_bus.set_addr(self.registers.get_hl(),self.registers.b); self.cycle=8; },
            0x71 => { self.memory_bus.set_addr(self.registers.get_hl(),self.registers.c); self.cycle=8; },
            0x72 => { self.memory_bus.set_addr(self.registers.get_hl(),self.registers.d); self.cycle=8; },
            0x73 => { self.memory_bus.set_addr(self.registers.get_hl(),self.registers.e); self.cycle=8; },
            0x74 => { self.memory_bus.set_addr(self.registers.get_hl(),self.registers.h); self.cycle=8; },
            0x75 => { self.memory_bus.set_addr(self.registers.get_hl(),self.registers.l); self.cycle=8; },
            0x36 => { self.registers.pc+=1; self.memory_bus.set_addr(self.registers.get_hl(),self.memory_bus.get_addr(self.registers.pc)); self.cycle=12; },
            0x0A => { self.registers.a=self.memory_bus.get_addr(self.registers.get_bc()); self.cycle=8; },
            0x1A => { self.registers.a=self.memory_bus.get_addr(self.registers.get_de()); self.cycle=8; },
            0xFA => { self.registers.pc+=1; byte=byte|(self.memory_bus.get_addr(self.registers.pc) as u16); self.registers.pc+=1; byte=byte|(self.memory_bus.get_addr(self.registers.pc) as u16)<<8; self.registers.a=self.memory_bus.get_addr(byte); self.cycle=16; },
            0x3E => { self.registers.pc+=1; self.registers.a=self.memory_bus.get_addr(self.registers.pc); self.cycle=8; },
            ...
            0x02 => {
                self.memory_bus.set_addr(self.registers.get_bc(),self.registers.a);
                self.cycle=8;
            },
            0x12 => {
                self.memory_bus.set_addr(self.registers.get_de(),self.registers.a);
                self.cycle=8;
            },
            0x77 => {
                self.memory_bus.set_addr(self.registers.get_hl(),self.registers.a);
                self.cycle=8;
            },
            0xEA => {
                self.registers.pc+=1;
                byte=byte|(self.memory_bus.get_addr(self.registers.pc) as u16);
                self.registers.pc+=1;
                byte=byte|(self.memory_bus.get_addr(self.registers.pc) as u16)<<8;
                self.memory_bus.set_addr(byte,self.registers.a);
                self.cycle=16;
            },
            0xF2 => {
                self.registers.a=self.memory_bus.get_addr(0xFF00|(self.registers.c as u16));
                self.cycle=8;
            },
            0xE2 => {
                self.memory_bus.set_addr(0xFF00+(self.registers.c as u16),self.registers.a);
                self.cycle=8;
            },
            0x3A => {
                self.registers.a=self.memory_bus.get_addr(self.registers.get_hl());
            ...
            0xE0 => {
                self.registers.pc+=1;
                byte=self.memory_bus.get_addr(self.registers.pc) as u16;
                self.memory_bus.set_addr(0xFF00+byte,self.registers.a);
                self.cycle=12;
            },
            0xF0 => {
                self.registers.pc+=1;
                byte=self.memory_bus.get_addr(self.registers.pc) as u16;
                self.registers.a=self.memory_bus.get_addr(0xFF00+byte);
                self.cycle=12;
            },
            //16 bit LD
            0x01 => {
            ...
            //To be reviewed
            0x08 => {
                self.registers.pc+=1; byte=byte|(self.memory_bus.get_addr(self.registers.pc) as u16);
                self.registers.pc+=1; byte=byte|(self.memory_bus.get_addr(self.registers.pc) as u16)<<8;
                self.memory_bus.set_addr(byte,(self.registers.sp&0x00ff) as u8); 
                self.memory_bus.set_addr(byte+1,(self.registers.sp>>8) as u8);
                self.cycle=20;
            },

            //Stack Operations
            //PUSH
            0xF5 => { self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.a); self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.f); self.cycle=16; },
            0xC5 => { self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.b); self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.c); self.cycle=16; },
            0xD5 => {
                self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.d);
                self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.e);
                self.cycle=16;
            },
            0xE5 => {
                self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.h);
                self.registers.sp-=1; self.memory_bus.set_addr(self.registers.sp,self.registers.l);
                self.cycle=16;
            },
            //POP
            0xF1 => {
                self.registers.f=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.registers.a=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.cycle=12;
            },
            0xC1 => {
                self.registers.c=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.registers.b=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.cycle=12;
            },
            0xD1 => {
                self.registers.e=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.registers.d=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.cycle=12;
            },
            0xE1 => {
                self.registers.l=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.registers.h=self.memory_bus.get_addr(self.registers.sp); self.registers.sp+=1;
                self.cycle=12;
            },
            ...
            0x86 => { self.registers.a=self.add8(self.registers.a,self.memory_bus.get_addr(self.registers.get_hl()),false); self.cycle=8; },
            0xC6 => { self.registers.pc+=1; byte=self.memory_bus.get_addr(self.registers.pc) as u16;
                self.registers.a=self.add8(self.registers.a,byte as u8,false); self.cycle=8; },
            //ADD WITH CARRY
            ...
            0x8E => { self.registers.a=self.add8(self.registers.a,self.memory_bus.get_addr(self.registers.get_hl()),true); self.cycle=8; },
            0xCE => { self.registers.pc+=1; byte=self.memory_bus.get_addr(self.registers.pc) as u16;
            ...
            0x96 => { self.registers.a=self.sub8(self.registers.a,self.memory_bus.get_addr(self.registers.get_hl()),false); self.cycle=8; },
            0xD6 => { self.registers.pc+=1; byte=self.memory_bus.get_addr(self.registers.pc) as u16;
            ...
            0x9E => { self.registers.a=self.sub8(self.registers.a,self.memory_bus.get_addr(self.registers.get_hl()),true); self.cycle=8; },
            0xDE => { self.registers.pc+=1; byte=self.memory_bus.get_addr(self.registers.pc) as u16;
            ...
            0xA6 => {self.registers.a&=self.memory_bus.get_addr(self.registers.get_hl()); self.registers.f=0x0; self.registers.set_h(); if self.registers.a==0x0 {self.registers.set_z();}self.cycle=8;},
            0xE6 => {self.registers.pc+=1; self.registers.a&=self.memory_bus.get_addr(self.registers.pc); self.registers.f=0x0; self.registers.set_h(); if self.registers.a==0x0 {self.registers.set_z();}self.cycle=8;},
            //OR
            ...
            0xB6 => {self.registers.a|=self.memory_bus.get_addr(self.registers.get_hl()); self.registers.f=0x0; if self.registers.a==0x0 {self.registers.set_z();}self.cycle=8;},
            0xF6 => {self.registers.pc+=1; self.registers.a|=self.memory_bus.get_addr(self.registers.pc); self.registers.f=0x0; if self.registers.a==0x0 {self.registers.set_z();}self.cycle=8;},
            //XOR
            ...
            0xAE => {self.registers.a^=self.memory_bus.get_addr(self.registers.get_hl()); self.registers.f=0x0; if self.registers.a==0x0 {self.registers.set_z();}self.cycle=8;},
            0xEE => {self.registers.pc+=1; self.registers.a^=self.memory_bus.get_addr(self.registers.pc); self.registers.f=0x0; if self.registers.a==0x0 {self.registers.set_z();}self.cycle=8;},
            //CP
            ...
            0xBE => { self.sub8(self.registers.a,self.memory_bus.get_addr(self.registers.get_hl()),false); self.cycle=8; },
            0xFE => { self.registers.pc+=1; byte=self.memory_bus.get_addr(self.registers.pc) as u16; self.sub8(self.registers.a,byte as u8,false); self.cycle=8; },
            ...
            0x34 => { flag=self.registers.get_c(); let hl_val=self.add8(self.memory_bus.get_addr(self.registers.get_hl()),0x01,false); self.memory_bus.set_addr(self.registers.get_hl(),hl_val); if flag {self.registers.set_c();} else{self.registers.reset_c();} self.cycle=12; },
            //DEC
            ...
            0x35 => { flag=self.registers.get_c(); let hl_val=self.sub8(self.memory_bus.get_addr(self.registers.get_hl()),0x01,false); self.memory_bus.set_addr(self.registers.get_hl(),hl_val); if flag {self.registers.set_c();} else{self.registers.reset_c();} self.cycle=12; },
            ...
            //ADD SP
            0xE8 => { self.registers.pc+=1; self.registers.sp=self.add16(self.registers.sp,self.memory_bus.get_addr(self.registers.pc) as u16,false);self.registers.reset_z(); self.registers.reset_n(); self.cycle=16; },
            //INC
            ...
            0xCB => {
                self.registers.pc+=1;
                opcode_cb=self.memory_bus.get_addr(self.registers.pc);
                ...
                match opcode_cb{
                    ...
                    0x36 => { self.registers.f=0x0; byte=self.registers.get_hl(); byte8=self.memory_bus.get_addr(byte) ; byte8=((byte8&0x0f)<<4)|((byte8&0xf0)>>4); self.memory_bus.set_addr(byte,byte8); if byte8==0 { self.registers.set_z(); } self.cycle=16;},
                    _ => (),
                };
            },
            //DAA
        ...
        self.registers.pc+=1;
        let gb_time=self.gb_clock.checked_mul(self.get_cycle_count() as u32).expect("1");
        match gb_time.checked_sub(execution_time_now.elapsed()){
            Some(x) => thread::sleep(x),
            None => (),
        }
        println!("Opcode : {} , Cycles : {} , Execution Time : {:?}",opcode,self.cycle,execution_time_now.elapsed());
    }
    pub fn print_registers(&self){
        println!("{:?}",self.registers);
    }
    pub fn bulk_load_addr(&mut self,byte_stream:Vec<u8>){
        let mut addr:u16=0x0;
        for byte in byte_stream{
            self.memory_bus.set_addr(addr,byte);
            addr+=1;
        }
    }
    ...
    pub fn get_addr_helper(&self,addr:u16)->u8{
        self.memory_bus.get_addr(addr)
    }
